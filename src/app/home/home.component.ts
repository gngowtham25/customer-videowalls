  ...
  constructor(public sanitizer: DomSanitizer, private fb: FormBuilder, private httpClient: HttpClient,private modalService: NgbModal,private router: Router) {
	 ...
	 requestRoomCheck(){
		 ...
		 this.httpClient.post(SERVER_URL+'/api/v1/room/ifRequestedRoomAlreadyThereForThisDevice',{'deviceId' : result1} , httpOptions)
            ...
            .subscribe(() => {
              this.httpClient.post(SERVER_URL+'/api/v1/room/isSalesmanAllotted', {'id':this.id4}, httpOptions)
                .subscribe((data: any) => {
					...
					.subscribe((data: any) => {
      				...
      				headers: new HttpHeaders({
							...
							this.subscription = Observable.interval(3000)
								...
								.subscribe(() => {
                  this.httpClient.post(SERVER_URL+'/api/v1/room/isSalesmanAllotted', {'id':id}, httpOptions)
                    .subscribe((data: any) => {
                      let id = JSON.parse(data.data).id;
                      if(id != undefined){
                        this.unsubscribeMe();
